...
import a from './ext.module.css'
import { polz } from './registr'
function CrExt(){

    const [inp1Value,setInp]=useState('')
    const [inp2Value,setInp2]=useState('')
    function Exist(i1,i2){
        // console.log(polz[0])
        if (localStorage.getItem("user")){
            let polz= JSON.parse(localStorage.getItem("user"))
            console.log(polz[0].elpoch)
            for(let i=0;i<polz.length;i++){
                console.log('b')
                if(polz[i].elpoch == i1){
                    if(polz[i].pas == i2){
    ...
    return(
        <div class={a.div}>
            ...
            <NavLink to='/'>
            <button class={a.btn_ext}><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            ...
            <input type="text" value={inp1Value} onChange={e=>setInp(e.target.value)} class={a.inp1} placeholder='Электронная почта'></input>
            <input type="text" value={inp2Value} onChange={e=>setInp2(e.target.value)} class={a.inp2} placeholder='Пороль'></input>
            <button class={a.btn_npporol}>Не помню пороль</button>
            <div class={a.btns}>
                <button onClick={()=>{
                console.log(inp1Value,inp2Value)
                Exist(inp1Value,inp2Value)
                }}class={a.btn_vh}>Вход в кабинет</button>
                <NavLink to='/'><button class={a.btn_reg}>Регистрация</button></NavLink>
            </div>
        </div>
    )
}
